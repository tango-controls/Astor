...
public class AstorUtil implements AstorDefs {
	...
	static void readAstorRes()
	{
		...
		try
		{
			...
			//	get Astor Property
			//----------------------------------
			String[]	propnames = { "Debug", "RloginUser" };
			DbDatum[]	datum = dbase.get_property("Astor", propnames);
			if (datum[0].is_empty()==false)
				debug = datum[0].extractBoolean();
			if (datum[1].is_empty()==false)
				rloginUser = datum[1].extractString();
			res_read = true;
		}
		catch(DevFailed e) {}
	}
	...
	static public boolean osIsUnix()
	{
		if (_osIsUnixTested==false)
		{
			try
			{
				String	os = AstorUtil.executeShellCmd("uname");
				//	Take Off Cr if exists
				int	end;
				if ((end=os.indexOf("\n"))>0)
					os = os.substring(0, end);
				//System.out.println("Running under " + os);
				_osIsUnix = (os.equals("Linux") || os.equals("SunOS") || os.equals("HP-UX"));
			}
			catch(Exception e)
			{
