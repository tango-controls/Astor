...
public class AstorUtil implements AstorDefs {
    ...
    {
        //  Query everytime because TANGO_HOST could have change.
        try
        {
            String[]	propnames = { "Tools" };
            tools = null;
            DbDatum[]	datum = ApiUtil.get_db_obj().get_property("Astor", propnames);
            if (!datum[0].is_empty())
                tools = datum[0].extractStringArray();
        }
        catch(DevFailed e){ /** Do Notothing */}
        return tools;
    }
    //===============================================================
    //===============================================================
	...
	//===============================================================
	//===============================================================
    static String[] known_tango_hosts = null;
	static void readAstorRes()
	{
		try
		{
			...
			if (!datum[++i].is_empty())
				known_tango_hosts = datum[i].extractStringArray();
			res_read = true;
		}
	...
	String[] getCollectionList(TangoHost[] hosts)
	{
        ...
        try
        {
            String[]	propnames = { "LastCollections" };
            DbDatum[]	datum = ApiUtil.get_db_obj().get_property("Astor", propnames);
            if (!datum[0].is_empty())
			{
                String[]	last = datum[0].extractStringArray();
