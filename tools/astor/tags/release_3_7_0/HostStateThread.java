...
public class HostStateThread extends Thread implements AstorDefs
{
	...
	public HostStateThread(AstorTree parent, TangoHost host)
	{
		...
		try {
			...
			if (data.is_empty()==false)
				host.use_events = (data.extractShort()!=0);
		}
		catch(DevFailed e){}

		//System.out.println(host.name() + "  " + host.use_events);
	}
	...
	private void subscribeChangeStateEvent()
	{
		try 
		{
			if (event_supplier==null)
				event_supplier = new TangoEventsAdapter(host.name());
	...
	private void unsubscribeStateEvent()
	{
		try
		{
			...
			System.out.println("unsubscribe event for " + stateAttr);
		}
		catch(DevFailed e)
		{
		...
		while (stop_it==false)
		{
			...
			if (host.do_polling)
			{
				...
				if (host.getName().equals("wow"))
					System.out.println("State = " + ApiUtil.stateName(devstate));
				*/
	...
	public synchronized void updateHost(int notifyd_state)
	{
		...
		System.out.println("Update for state = " + notifyd_state);
		parent.updateState();
	}
	//======================================================================
	//======================================================================
	...
	public synchronized void updateHost(String[] running, String[] stopped)
	{
		...
		if (prev_running.length != running.length ||
			prev_stopped.length != stopped.length )
			changed = true;
		else
		{
		...
		//	Check if some of them have disapeared
		for (int i=host.nbServers()-1 ; i>=0; i--)
		{
			TangoServer	server = host.getServer(i);
			boolean		found = false;
			for(int j=0 ; !found && j<running.length ; j++)
				found = server.getName().equals(running[j]);
		...
		for(int i=0 ; i<running.length ; i++)
			try {
				...
				//if (host.getName().equals("corvus"))
				//	System.out.println(running[i] + " : " + true);
			}
			catch(DevFailed e) { }
		...
		for(int i=0 ; i<stopped.length ; i++)
			try {
				...
				//if (host.getName().equals("corvus"))
				//	System.out.println(stopped[i] + " : " + false);
			}
			catch(DevFailed e) { }
...
/***
if (host.getName().equals("wow"))
System.out.print("Checking notifyd state");
*****/
			if (running)
...
/*****/
	 ...
	 */
	//=========================================================================
	class StateEventListener implements ITangoPeriodicListener, ITangoChangeListener
	{
		//=====================================================================
		//=====================================================================
    	public void change(TangoChangeEvent event) {
			...
			catch (DevFailed e)
			{
				...
				{
					notifyd_state = unavailable;
					System.out.println("HostStataThread.StateEventListener" +
	 ...
	 */
	//=========================================================================
	class ServerEventListener implements ITangoPeriodicListener, ITangoChangeListener
	{
		//=====================================================================
		//=====================================================================
    	public void change(TangoChangeEvent event) {
