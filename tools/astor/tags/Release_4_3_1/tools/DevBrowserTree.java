   ...
   public DevBrowserTree(DevBrowser parent) throws DevFailed
   {
    ...
    private boolean createChildNodes(DefaultMutableTreeNode node, String[] str)
    {
        ...
        for (int i=0 ; i<str.length ; i++)
            if (node.getChildAt(i).toString().equals(str[i])==false)
                create = true;
        return create;
    }
	//===============================================================
	...
	private void createAliasNodes(DefaultMutableTreeNode root) throws DevFailed
	{
		...
		for (int i=0 ; i<aliases.length ; i++)
		{
            ...
            try
            {
                ...
                al_node =  new DefaultMutableTreeNode(new BrowserDevice(devname, aliases[i]));
    ...
    private void createServerNodes(DefaultMutableTreeNode root) throws DevFailed
    {
        ...
        {
            //	Create a node for domain
            d_node =  new DefaultMutableTreeNode(servers[d]);
    ...
    private void createInstanceNodes(DefaultMutableTreeNode node)
    {
        try
        {
            ...
            if (createChildNodes(node, instancies)==false)
                return;
    ...
    private void createDeviceNodesFromServer(DefaultMutableTreeNode node)
    {
        try
        {
            DefaultMutableTreeNode	d_node = null;
            ...
            //  Check if something has changed.
            if (createChildNodes(node, devices)==false)
                return;
    ...
    private void createDeviceNodes(DefaultMutableTreeNode root) throws DevFailed
    {
        ...
        {
            //	Create a node for domain
            d_node =  new DefaultMutableTreeNode(domains[d]);
            d_node.add(new DefaultMutableTreeNode("dummy"));
            c_node.add(d_node);
        }
    }
	...
	private void createMemberNodes(String tango_path, DefaultMutableTreeNode node)
	{
		try
		{
            ...
            //  Check if something has changed.
            if (createChildNodes(node, members)==false)
                return;
	...
	public void expandedPerfomed(TreeExpansionEvent evt)
	{
        ...
        if (path[1].toString().equals("Devices"))
        {
            switch (path.length)
            {
            case DOMAIN:
        ...
        if (path[1].toString().equals("Aliases"))
        {
            if (path.length==3)
                createAttributeNodes(node);
        }
	...
	private void treeMouseClicked (java.awt.event.MouseEvent evt)
	{
        ...
        if ((mask & evt.BUTTON3_MASK)!=0)
		{
             ...
             if (o instanceof BrowserAttribute)
            {
             ...
             if (o instanceof BrowserDevice)
             {
                 if (displayDeviceInfo((BrowserDevice)o))
                     dev_menu.showMenu(evt, obj_has_polling);
             }
             else
             if (o instanceof BrowserServer)
             {
                 if (displayDeviceInfo(((BrowserServer)o).dev))
                     serv_menu.showMenu(evt, obj_has_polling);
             }
		}
		else
	...
	void showProfiler()
    {
        ...
        {
            app_util.PopupError.show(browser, e);
        }
    }
	//======================================================
	//======================================================
    ...
    private String getAttPollingInfo()
    {
        ...
        if (o instanceof BrowserAttribute)
        {
            try
            {
                ...
                {
                    String   s = poll_status[i].substring(header.length(),
                                    poll_status[i].indexOf('\n'));
		...
		public Component getTreeCellRendererComponent(
			...
			else
			{
                ...
                if (node.getUserObject() instanceof String)
                {
                    ...
                    if (obj.toString().equals("Devices"))
                        setIcon(dev_icon);
                    else
                    if (obj.toString().equals("Aliases"))
                        setIcon(dev_icon);
                    else
                    {
				...
				if (node.getUserObject() instanceof BrowserAttribute)//getAttribute(node)!=null)
				{
					setFont(fonts[ATTR]);
					setIcon(attr_icon);
				}
                else
                if (node.getUserObject() instanceof BrowserDevice)
                {
                    setFont(fonts[DEVICE]);
                    setIcon(dev_icon);
                }
                else
                if (node.getUserObject() instanceof BrowserServer)
                {
                    setFont(fonts[DEVICE]);
                    setIcon(serv_icon);
                }
