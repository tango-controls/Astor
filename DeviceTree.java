...
public class DeviceTree extends JTree  implements AstorDefs
{
	...
	public void collapsedPerfomed(TreeExpansionEvent e)
	{
		...
		if (path.getPathCount()==1)
		{
			...
			app_util.PopupMessage.showImage(appli, cs.toString(), "/admin/astor/img/tango_icon.jpg");
		}
	}
	//======================================================
	//======================================================
	Vector getNotCtrlServers()
	{
		...
		for (int i=0 ; i<nb_serv ; i++)
		{
			...
			try
			{
				...
				for (int j=0 ; j<nb_inst ; j++)
				{
					...
					if (info.controlled==false)
						//	Starter cannot controle itself.
						if (servname.equals("Starter")==false)
							v_serv.add(server);
				}
			}
			catch(DevFailed e){
		...
		for (int i=0 ; i<v_serv.size() ; i++)
		{
			...
			try
			{
				...
				String	date = info2.last_exported;
				v_date.add(date);
			}
			catch (DevFailed e){
				v_date.add(e.errors[0].desc);
			}
		}
